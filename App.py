...
# Check if we're in the main Gunicorn worker (not a fork)
# This prevents multiple schedulers when using multiple workers
if os.environ.get('WERKZEUG_RUN_MAIN') != 'true' and os.getpid() == os.getppid():
    # Running in development mode - only start in main process
    try:
    ...
    # Running in Gunicorn - only start in one worker
    # Use an environment variable to coordinate
